...
public class Mesh 
{
	 ...
	 */
	public Mesh(  boolean managed, boolean isStatic, int maxVertices, int maxIndices, VertexAttribute ... attributes )
	{
		...
		if( usesDirectBuffers == false )
		{
			...
			vertices = FloatBuffer.wrap( new float[maxVertices * this.attributes.vertexSize/4] );
	...
	private void renderVBO( int primitiveType, int offset, int count )
	{
		...
		for( int i = 0; i < numAttributes; i++ )
		{
			...
			if( attribute.usage == Usage.Position )
			{
				...
				gl.glVertexPointer( attribute.numComponents, GL11.GL_FLOAT, attributes.vertexSize, attribute.offset );
				continue;
			}
			...
			{
				gl.glEnableClientState( GL11.GL_COLOR_ARRAY );
				int colorType= GL11.GL_UNSIGNED_BYTE;
				gl.glColorPointer( attribute.numComponents, colorType, attributes.vertexSize, attribute.offset );
				continue;
			}
			if( attribute.usage == Usage.Normal )
			{
				...
				gl.glNormalPointer( GL11.GL_FLOAT, attributes.vertexSize, attribute.offset );
				continue;
			}
			...
			if( attribute.usage == Usage.TextureCoordinates )
			{
				...
				gl.glTexCoordPointer( attribute.numComponents, GL11.GL_FLOAT, attributes.vertexSize, attribute.offset );
				textureUnit++;
				continue;
			}
		}
	...
	private void renderVA( int primitiveType, int offset, int count )
	{
		...
		for( int i = 0; i < numAttributes; i++ )
		{
			...
			if( attribute.usage == Usage.Position )
			{
				...
				gl.glVertexPointer( attribute.numComponents, GL11.GL_FLOAT, attributes.vertexSize, vertices );
				continue;
			}
			...
			if( attribute.usage == Usage.ColorPacked )
			{
				...
				int colorType= GL11.GL_COLOR_ARRAY;
			...
			if( attribute.usage == Usage.Normal )
			{
				gl.glEnableClientState( GL11.GL_NORMAL_ARRAY );
				vertices.position( attribute.offset );
				gl.glNormalPointer( GL11.GL_FLOAT, attributes.vertexSize, vertices );
				continue;
			}
			...
			if( attribute.usage == Usage.TextureCoordinates )
			{
				...
				gl.glEnableClientState( GL11.GL_TEXTURE_COORD_ARRAY );
				vertices.position( attribute.offset );
				gl.glTexCoordPointer( attribute.numComponents, GL11.GL_FLOAT, attributes.vertexSize, vertices );
				textureUnit++;
				continue;
			}
		}
	...
	public void render( ShaderProgram shader, int primitiveType, int offset, int count )
	{
		...
		for( int i = 0; i < numAttributes; i++ )
		{
			...
			shader.setVertexAttribute( attribute.alias, attribute.numComponents, GL11.GL_FLOAT, false, attributes.vertexSize, attribute.offset );
		}
