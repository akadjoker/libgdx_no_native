...
import static com.badlogic.gdx.graphics.Sprite.SPRITE_SIZE;
import static com.badlogic.gdx.graphics.Sprite.VERTEX_SIZE;
...
public class SpriteCache{
	...
	private final Matrix combinedMatrix = new Matrix();
	...
	mesh=new	Mesh(true, false,size * (useIndices ? 4 : 6), useIndices ? size * 6 : 0,
	...
	public int endCache () {
		...
		if (cache.textures == null) {
			...
			FloatBuffer vertices =(FloatBuffer) mesh.getVerticesBuffer();
			vertices.position(0);
	...
	public void add (Texture texture, float[] vertices, int offset, int length) {
		...
		((FloatBuffer)mesh.getVerticesBuffer()).put(vertices, offset, length);
	}
	...
	public void setTransformMatrix (Matrix transform) {
		if (drawing) throw new IllegalStateException("Can't set the matrix within begin/end.");
...
String fragmentShader = "precision mediump float;\n" +
		...
		ShaderProgram shader = new ShaderProgram(Gdx.graphics.getGL20(),vertexShader, fragmentShader,true);
