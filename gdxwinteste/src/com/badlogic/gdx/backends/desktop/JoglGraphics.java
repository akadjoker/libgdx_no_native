...
public final class JoglGraphics implements Graphics, RenderListener
{
	...
	{					
		...
		InputStream in;
		try {
			in = new FileInputStream( jHandle.file() );
	...
	{	
		try
		{
			BufferedImage img = (BufferedImage)ImageIO.read( ((JoglFileHandle)file).file() );
			return new JoglPixmap( img );
		}
		catch( Exception ex )
		{
	...
	public Texture newTexture(FileHandle file, boolean managed)
	{
		...
		if (pixmap!=null)
		{
		return new JoglTexture( (BufferedImage)pixmap.getNativePixmap(), TextureFilter.Linear, TextureFilter.Linear, TextureWrap.Wrap, TextureWrap.Wrap, managed );
		} else
			return null;
