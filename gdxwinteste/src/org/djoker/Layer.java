...
public class Layer {
	 ...
	 * @throws GdxRuntimeException Indicates a failure to parse the XML layer
	 */
	public Layer(TiledMap map, Element element) throws GdxRuntimeException {
		this.map = map;
		...
		if (encoding.equals("base64") && compression.equals("gzip")) {
			try {
				...
				throw new GdxRuntimeException("Unable to decode base 64 block");
			}
		} else {
			throw new GdxRuntimeException("Unsupport tiled map type: "+encoding+","+compression+" (only gzip base64 supported)");
		}
	}
