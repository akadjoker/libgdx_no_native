...
public class Image implements Renderable {
    ...
    private int filter = GL10.GL_LINEAR;
	 ...
	 */
	protected Image(GDXRenderer render)
	{
		GL=render;
	}
	 ...
	 *            The texture to use
	 */
	public Image(GDXRenderer render,Texture texture) {
	 ...
	 */
	public Image(GDXRenderer render,String ref)
	{
		this(render,ref, false);
	}
	 ...
	 * @throws GdxRuntimeException Indicates a failure to load the image
	 */
	public Image(GDXRenderer render,String ref, Color trans)
	{
		this( render,ref, false, FILTER_LINEAR, trans);
	}
	 ...
	 * @throws GdxRuntimeException Indicates a failure to load the image
	 */
	public Image(GDXRenderer render,String ref, boolean flipped)  {
		this(render,ref, flipped, FILTER_LINEAR);
	}
	 ...
	 * @throws GdxRuntimeException Indicates a failure to load the image
	 */
	public Image(GDXRenderer render,String ref, boolean flipped, int filter) 
 {
		this(render,ref, flipped, filter, null);
	}
	 ...
	 * @throws GdxRuntimeException Indicates a failure to load the image
	 */
	public Image(GDXRenderer render,String ref, boolean flipped, int f, Color transparent)  {
		this.filter = f == FILTER_LINEAR ? GL10.GL_LINEAR : GL10.GL_NEAREST;
		...
		this.GL=render;
			...
			texture =Gdx.graphics.newTexture(ref, false);// InternalTextureLoader.get().getTexture(ref, flipped, filter, trans);
	...
	public void setFilter(int f) {
		...
		GL.glTexParameteri(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MIN_FILTER, filter); 
        GL.glTexParameteri(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_MAG_FILTER, filter); 
	}
	...
	/*
		if (GL.canTextureMirrorClamp()) {
        	GL.glTexParameteri(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_WRAP_S, GL10.GL_MIRROR_CLAMP_TO_EDGE_EXT);
        	GL.glTexParameteri(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_WRAP_T, GL10.GL_MIRROR_CLAMP_TO_EDGE_EXT);
        } else {
        	GL.glTexParameteri(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_WRAP_S, GL10.GL_CLAMP);
        	GL.glTexParameteri(GL10.GL_TEXTURE_2D, GL10.GL_TEXTURE_WRAP_T, GL10.GL_CLAMP);
        }
	...
	//	col.bind();
		...
		{
		///	GL.glEnable(GL10.GL_COLOR_SUM_EXT);
		//	GL.glSecondaryColor3ubEXT((byte)(col.r * 255), 
			//										 (byte)(col.g * 255), 
			//										 (byte)(col.b * 255));
		}
		
		GL.glTexEnvi(GL10.GL_TEXTURE_ENV, GL10.GL_TEXTURE_ENV_MODE, GL10.GL_MODULATE);
	...
	//	if (GL.canSecondaryColor()) {
	//		GL.glDisable(GL10.GL_COLOR_SUM_EXT);
	//	}
	...
	public Color getColor(int x, int y) {
		...
		int xo = (int) (textureOffsetX * texture.getImageWidth());
		int yo = (int) (textureOffsetY * texture.getImageHeight());
		...
		int offset = x + (y * texture.getImageWidth());
		offset *=4;// texture.hasAlpha() ? 4 : 3;
	 ...
	 * 
	 * @throws GdxRuntimeException Indicates a failure to release resources on the graphics card
	 */
	public void destroy() throws GdxRuntimeException {
		if (isDestroyed()) {
			return;
		}
