...
public class TileSet {
	 ...
	 * @throws GdxRuntimeException Indicates a failure to parse the tileset
	 */
	public TileSet(GDXRenderer render,TiledMap map, Element element, boolean loadImage) throws GdxRuntimeException {
		this.map = map;
		...
		if ((source != null) && (!source.equals(""))) {
			try {
				InputStream in =Gdx.files.readFile(map.getTilesLocation() + "/" + source,FileType.Internal);// ResourceLoader.getResourceAsStream(map.getTilesLocation() + "/" + source);
				...
				throw new GdxRuntimeException("Unable to load or parse sourced tileset: "+this.map.tilesLocation+"/"+source);
			}
		}
        ...
        if(tileWidthString.length() == 0 || tileHeightString.length() == 0) {
            throw new GdxRuntimeException("TiledMap requires that the map be created with tilesets that use a " +
		...
		if (loadImage) {
			Image image = new Image(render,map.getTilesLocation()+"/"+ref,false,Image.FILTER_NEAREST,trans);
