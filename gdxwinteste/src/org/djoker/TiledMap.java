...
public class TiledMap {
	 ...
	 * @throws GdxRuntimeException Indicates a failure to load the tilemap
	 */
	public TiledMap(GDXRenderer render,String ref) throws GdxRuntimeException {
		this(render,ref, true);
	}
	 ...
	 * @throws GdxRuntimeException Indicates a failure to load the tilemap
	 */
	public TiledMap(GDXRenderer render,String ref, boolean loadTileSets) throws GdxRuntimeException 
	{
		this.loadTileSets = loadTileSets;
		ref = ref.replace('\\','/');
		//load(ResourceLoader.getResourceAsStream(ref), ref.substring(0,ref.lastIndexOf("/")));
		load(render,Gdx.files.readFile(ref,FileType.Internal),  ref.substring(0,ref.lastIndexOf("/")));
	}
	 ...
	 * @throws GdxRuntimeException Indicates a failure to load the tilemap
	 */
	public TiledMap(GDXRenderer render,String ref, String tileSetsLocation) throws GdxRuntimeException 
	{
		//load(ResourceLoader.getResourceAsStream(ref), tileSetsLocation);
		load(render,Gdx.files.readFile(ref,FileType.Internal), tileSetsLocation);
	}
	 ...
	 * @throws GdxRuntimeException Indicates a failure to load the tilemap
	 */
	public TiledMap(GDXRenderer render,InputStream in) throws GdxRuntimeException {
		load(render,in, "");
	}
	 ...
	 * @throws GdxRuntimeException Indicates a failure to load the tilemap
	 */
	public TiledMap(GDXRenderer render,InputStream in, String tileSetsLocation) throws GdxRuntimeException {
		load(render,in, tileSetsLocation);
	}
   ...
   public Image getTileImage(int x, int y, int layerIndex) {
	 ...
	 * @throws GdxRuntimeException Indicates a failure to parse the map or find a tileset
	 */
	private void load(GDXRenderer render,InputStream in, String tileSetsLocation) throws GdxRuntimeException {
		...
		try {
			...
			if (!orient.equals("orthogonal")) {
				throw new GdxRuntimeException("Only orthogonal maps supported, found: "+orient);
			}*/
			...
			if (loadTileSets) {
				...
				for (int i=0;i<setNodes.getLength();i++) {
					...
					tileSet = new TileSet(render,this, current, !headless);
					tileSet.index = i;
			...
			throw new GdxRuntimeException("Failed to parse tilemap", e);
		}
	}
	...
	protected class ObjectGroup {
	   ...
	   * @throws GdxRuntimeException Indicates a failure to parse the XML group
	   */
	  public ObjectGroup(Element element) throws GdxRuntimeException {
	 ...
	 * @throws GdxRuntimeException Indicates a failure to parse the XML group
	 */
	 public GroupObject(Element element) throws GdxRuntimeException {
			name = element.getAttribute("name");
